...
function init() {
  L10N.l('ko')
  L10N.append(localText);

  siteData.ll.en = Object.keys(L10N.data).filter(e => e.includes('local')).map(e => e.replace('local_', ''));
  siteData.ll.trans = siteData.ll.en.map(e => L10N.t(`local_${e}`));

  siteData.schPre.keys = Object.keys(L10N.data).filter(e => e.includes('sch_prefix'));
  siteData.schPre.trans = siteData.schPre.keys.map(e => L10N.t(e));
...
function imageSave() {
  ...
  const cssUrl = 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@latest/css/all.min.css';
  document.head.querySelector(`link[rel="stylesheet"][href="${cssUrl}"]`)?.remove();
...
function addForm(tempElem) {
  const dispElem = document.querySelector('.char_info_wrap');
  let form = null;
  let menu = document.querySelector('ul.side');
  if (document.forms.length == 0) {
    document.querySelector('.char_wrap').style.display = 'none';
    dispElem.insertAdjacentElement('beforeend', tempElem.querySelector('form'));
    form = document.forms[0];
    form.querySelector('label[for="chrL"]').insertAdjacentElement('afterend', (() => {
      let s = document.createElement('select');
      s.name = 'chrL';
      s.id = 'chrL';
      siteData.ll.trans.map((str, i) => {
        let o = document.createElement('option');
        o.value = siteData.ll.en[i];
        o.innerText = str;
    ...
    form.querySelector('label[for="chrSchPref"]').insertAdjacentElement('afterend', (() => {
      ...
      s.name = 'chrSch';
      s.id = 'chrSch';
      siteData.schPre.trans.map((str, i) => {
        let o = document.createElement('option');
        o.value = i;
        o.innerText = str;
        if (i == 0) o.selected = true;
...
function reflash(data) {
  ...
  document.querySelector('.char_info_wrap').remove();
  document.body.insertAdjacentHTML('afterbegin', `<div class="char_info_wrap" id="job1"></div>`);
  const dispElem = document.querySelector('.char_info_wrap');

  //parse template;

  return Promise.resolve(new DOMParser().parseFromString(siteData.t, "text/html").body)
    .then(tempElem => {
      dispElem.insertAdjacentElement('afterbegin', tempElem.querySelector('.char_wrap'));
      if (!document.querySelector('ul.side')) document.body.insertAdjacentElement('beforeend', tempElem.querySelector('ul.side'));
      document.querySelector('ul.side li:nth-child(1)').onclick = () => addForm(tempElem);
      document.querySelector('ul.side li:nth-child(2)').onclick = () => imageSave();
      genChrCard(data)
    })

}

window.onload = () => init().then(() => reflash(siteData.chrT));
    ...
    switch (i) {
      ...
      case 9:
        ...
        if (e) {
          ...
          img.src = i == 0 ? './asset/01_m_shiroko.png' : data.d[5] != 'unset' ? `./asset/sch/School_Icon_${data.d[5].toUpperCase()}.png` : '';
        };
          ...
          if (i % 2 == 1) {
            for (let j = 0; j < 2; j++) {
              ...
              if (j == 0) {
                ...
                elm.innerText = i == 3 ? e : i == 5 ? L10N.t(`local_${e}`) : `${document.getElementById('lo').innerHTML} ${siteData.schPre.trans[parseInt(e)]}`;
              };
              if (i > 3) {
      ...
      case 20:
        ...
        if (i > 13 && i < 19) {
          document.querySelector(e == false && i > 13 && i < 19 ? `dl.char_info :nth-child(${i - 10})` : i == 19 ? '.sch_logo' : 'canvas').style.display = 'none';
        }
